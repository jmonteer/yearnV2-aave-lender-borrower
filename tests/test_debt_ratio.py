...
def test_increase(vault, strategy, gov, token, token_whale, borrow_token, borrow_whale, yvault):
    token.approve(vault, 2 ** 256 - 1, {"from": token_whale})
    vault.deposit(20 * (10**token.decimals()), {"from": token_whale})
    vault.updateStrategyDebtRatio(strategy, 5_000, {"from": gov})

    strategy.harvest({"from": gov})
    assert vault.strategies(strategy).dict()["totalDebt"] == 10 * (10**token.decimals())
    ...
    strategy.harvest({"from": gov})
    assert vault.strategies(strategy).dict()["totalDebt"] >= 20 * (10**token.decimals())
...
def test_decrease(vault, strategy, gov, token, token_whale):
    ...
    assert vault.strategies(strategy).dict()["totalDebt"] < 15 * (10**token.decimals())
    assert vault.strategies(strategy).dict()["totalLoss"] == 0
